...
function processNode (node, parentKey, props) {
  ...
  if (isText(nodeName)) {
    const key = `${parentKey}_text`
    return (<Text key={key} style={props.style} numberOfLines={props.numberOfLines}>{node.value}</Text>)
  }
  ...
  if (isBlockElement(nodeName)) {
    ...
    if (lastInlineNodes.length > 0) {
      children.push((
        <Text numberOfLines={props.numberOfLines} key={`${key}_last_inline`} style={[styleForTag(nodeName), props.style]}>{lastInlineNodes}</Text>))
    }
    return (
      <View key={key}>
        {children}
